    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (v.id) {
            ...
            R.id.btn_move_activity_with_data -> {
                ...
                moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_NAME, "arawr")
                moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_AGE, "17")
                startActivity(moveWithDataIntent)
            }
            R.id.btn_dial_number->{
                val phoneNumber = "0895809092050"
                val dialPhoneIntent = Intent(Intent.ACTION_DIAL, Uri.parse("tel:$phoneNumber"))
                startActivity(dialPhoneIntent)
            }
            R.id.btn_move_activity_with_object->{
                val person = Person("arawr", "nismararamadhani@gmail.com", "Malang")
